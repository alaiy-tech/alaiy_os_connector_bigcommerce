...
reads the configured intervals from BigCommerce Connector Settings and enqueues
...
def check_and_enqueue():
    if not frappe.db.exists("DocType", "BigCommerce Sync Log"):
        return
    ...
    _maybe_enqueue(
        interval_setting=settings.bc_pull_sync_interval or "Disabled",
        sync_type="pull",
        enqueue_fn="alaiy_os_connector_bigcommerce.bigcommerce.sync.run_pull_sync",
    )
    _maybe_enqueue(
        interval_setting=settings.bc_push_sync_interval or "Disabled",
        sync_type="push",
        enqueue_fn="alaiy_os_connector_bigcommerce.bigcommerce.sync.run_push_sync",
    )
