...
from alaiy_os_connector_bigcommerce.bigcommerce.sync import get_or_create_log
...
def trigger_pull_sync():
    ...
    frappe.enqueue(
        "alaiy_os_connector_bigcommerce.bigcommerce.sync.run_pull_sync",
        queue="long",
        timeout=600,
        trigger="manual",
        log_name=log.name,
...
def trigger_push_sync():
    ...
    frappe.enqueue(
        "alaiy_os_connector_bigcommerce.bigcommerce.sync.run_push_sync",
        queue="long",
        timeout=600,
        trigger="manual",
        log_name=log.name,
...
def get_sync_status(sync_type=None):
    """
    Return the most recent BigCommerce Sync Log rows, newest first.
