...
class BigCommerceConnectorSettings(Document):
    def validate(self):
        ...
        old_enabled = frappe.db.get_single_value(
            "BigCommerce Connector Settings", "is_enabled"
        ) or 0
        self.flags.bc_just_enabled = bool(self.is_enabled and not old_enabled)
        self.flags.bc_just_disabled = bool(not self.is_enabled and old_enabled)
    ...
    def on_update(self):
        ...
        if self.flags.bc_just_enabled:
            self._on_first_enable()
        elif self.flags.bc_just_disabled:
            self._on_disable()
    ...
    def _on_first_enable(self):
        ...
        # registering BigCommerce webhooks once real credentials are saved.
        pass
    ...
    def _sync_registry_is_enabled(self):
        if frappe.db.exists("OS Connector Registry", "bigcommerce"):
            frappe.db.set_value(
                "OS Connector Registry", "bigcommerce", "is_enabled", self.is_enabled
            )
